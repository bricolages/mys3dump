...
interface RowFormatter {
    ...
    static RowFormatter newInstance(String format, ResultSetSchema rsSchema) throws SQLException {
        if (format.equals("csv")) return new CSVRowFormatter(rsSchema.columnTypes.length);
        if (format.equals("tsv")) return new TSVRowFormatter(rsSchema.columnTypes.length);
